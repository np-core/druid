...
class ProteinModel(PoreLogger):
    ...
    def regularize_mesh_surface(self, mesh, resolution, detail="normal"):
        ...
        """ Regularize a protein surface mesh.
        - based on code from the PyMESH documentation
        """

        self.logger.info("Regularizing surface mesh using PyMesh")
        ...
        while True:
            ...
            num_vertices = mesh.num_vertices
            self.logger.info(f"Iteration {count}: {num_vertices}")
            count += 1
            if count > 10:
                break
...
class Tricorder(PoreLogger):
    ...
    def compute_apbs(self, vertices, pdb_file: Path):
        ...
        args = [
            ...
            "--apbs-input",
            tmp_file_base+".in",
            f"{self.protein_model.pdb_id}_{self.protein_model.chains}",
            tmp_file_base
        ]
...
def compute_normal(vertex, face):
    """ Compute the normals of a closed shape.
